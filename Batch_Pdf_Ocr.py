...
# --- 輸入 / 輸出路徑 ---
PDF_INPUT_DIR = "pdfs/"      # 你嘅 PDF 檔案存放嘅資料夾
OUTPUT_BASE_DIR = "output_pdfs/"  # 所有處理結果嘅基底資料夾
...
# 'single'   = 所有 block 儲存到一個 .txt 檔案 (會喺每一頁嘅資料夾入面)
...
for filename in os.listdir(PDF_INPUT_DIR):
    if filename.lower().endswith('.pdf'):
        ...
        # predict 方法可以直接處理 PDF，並回傳一個包含每一頁結果嘅 list
            ...
            if BLOCK_TEXT_SAVE_MODE == 'single':
                ...
                with open(single_txt_path, "w", encoding="utf-8") as f:
                    for i, block in enumerate(parsing_res):
                        ...
                        f.write("\n\n")
                print(f"    [✓] 已將所有 Block 合併儲存到單一 .txt 檔案")
